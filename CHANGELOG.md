...
## v1.2.0-preview - 2026-06-06

架构预告版本，当前为实施规划，不代表代码已完成发布。

### 规划

- 计划新增“业绩”底部导航入口，替换当前“小记”主入口。
- 计划新增 `bx_work_payment` 收款账本，支持按实际收款月份统计净业绩。
- 计划新增 `bx_work_commission` 提成事件账本，支持 70/30 先发、冻结、释放、扣回和调整。
- 计划将工资结算从订单参与人一次性计算，改为从提成事件汇总。
- 计划新增小程序内管理中心，管理员从“我的”页基于 openid 免密码进入。
- 计划加强权限、幂等、事务、金额精度和审计设计。

### 文档

- 新增 [v1.2.0 更新预告](docs/architecture/v1.2.0-update-preview.md)。
