...
小猫助手游戏UX动画优化：签到发光、资源数字跳动、浮动奖励弹出、卡片交错入场。

- 资源数字增加 countUp 跳动动画，金币/素材/灵感值变化时有 scale(1.3) 弹跳反馈。
- 签到条未签到时增加呼吸发光动画（checkinPulse），吸引用户点击。
- 新增浮动数字弹出组件，领取离线收益/签到时在按钮上方显示 +N 浮动消散效果。
- 页面各区块交错入场（slideUp 0.1s 递增延迟），首次加载有层次感。
- 猫咪头像增加 catFloat 浮动动画，持续上下飘动增加活力。
- 签到、小游戏卡片增加:active 缩放反馈，点击手感更佳。
- 新增 CSS 自定义属性统一主题色管理，减少硬编码。
